(() => {
  ...
  function loadConfigOverrides() {
    try {
      ...
      if (saved && typeof saved === "object") {
        ...
        if (saved.rpcUrl) {
          CONFIG.rpcUrl = trimOrEmpty(saved.rpcUrl) || DEFAULT_MAINNET_RPC;
        }
      }
    } catch (_) {}
  ...
  function isConfigReady() {
    return isValidAddress(CONFIG.tokenAddress) && (isValidAddress(CONFIG.vaultAddress) || isValidAddress(CONFIG.factoryAddress));
  }
  ...
  const LEGION_NAMES = { 1: "AI军团", 2: "电车军团", 3: "比特军团", 4: "交易所军团", 5: "军工AI军团" };
  const LEGION_ICONS = {
    1: "./素材/图标/AI军团.webp",
    2: "./素材/图标/电车军团.webp",
    3: "./素材/图标/比特军团.webp",
  ...
  async function loadUserData() {
    ...
    const [u, balance, seasonId] = await Promise.all([state.readVault.users(state.userAddress), state.readToken.balanceOf(state.userAddress), state.readVault.currentSeasonId()]);
  ...
  function renderLeaderboardRows(list) {
    ...
    return list
      .map((x, i) => {
        ...
        return `<div class="lb-row" data-legion="${x.legionId}"><div class="lb-rank ${rankClass}">${rank}</div><div class="lb-icon">${icon ? `<img src="${icon}" alt="${name}" />` : ""}</div><div class="lb-main"><div class="lb-name">${name}</div><div class="lb-bar"><div class="lb-bar-fill" style="width:${width}%"></div></div></div><div class="lb-score">${formatInteger(x.heat)}</div></div>`;
      })
      .join("");
  }
  function renderHero() {
  ...
  function renderDrawer() {
    ...
    el.drawerContent.innerHTML = `<p class="empty-text">旧背包与战术包面板已停用，当前前端仅保留 FLAP STOCK 新玩法读写入口。</p>`;
  }
  ...
  function openSeatDetail() {
    showToast("旧席位详情已停用，当前前端仅保留 FLAP STOCK 新玩法入口", "error");
  }
  ...
  function unsupportedLegacyAction() {
    showToast("旧版世界杯玩法入口已停用，当前前端仅接入 FLAP STOCK 新玩法", "error");
  }
    ...
    openModal("军团玩法", `
      <div class="rules-modal rules-layout">
        <section class="rules-hero rules-top">
          <div><p class="eyebrow">FLAP STOCK 玩法总览</p><h3>五大军团、军衔成长、热度冲榜、赛季分奖与 24 小时股王</h3><p>先看加入规则，再看热度与资格，最后看 2 小时单轮结算、12 轮汇总成 24 小时股王周期，以及军团长竞争。</p></div>
        </section>
        <section class="rules-onboard">
          <div><p class="rules-section-title">五大固定军团</p><div class="rules-legions-grid"><span class="rules-legion-pill">01 AI军团</span><span class="rules-legion-pill">02 电车军团</span><span class="rules-legion-pill">03 比特军团</span><span class="rules-legion-pill">04 交易所军团</span><span class="rules-legion-pill">05 军工AI军团</span></div></div>
        ...
        <section class="rules-core">
          ...
          <div class="rules-stack">
            ...
            <article class="rules-visual-card"><strong>军团长产生与挑战</strong><div class="rules-eq"><span>初始军团长</span><b>首位成功挑战者</b></div><div class="rules-eq"><span>挑战门槛</span><b>投资总监及以上</b></div><div class="rules-eq"><span>比较标准</span><b>本军团历史贡献更高</b></div><div class="rules-eq"><span>成功结果</span><b>成为军团长并获得 30 分钟保护</b></div><p>锁榜期和未结算阶段不可挑战，避免赛果被临门改写。</p></article>
          </div>
        </section>
        <section class="rules-settlement">
          <article class="rules-visual-card"><strong>赛季流程</strong><div class="rules-flow"><div class="rules-flow-step"><label>01</label><span>赛季开始</span><b>2 小时一轮</b></div><div class="rules-flow-step"><label>02</label><span>锁榜阶段</span><b>最后 5 分钟</b></div><div class="rules-flow-step active"><label>03</label><span>本轮释放</span><b>当前奖池 40%</b></div><div class="rules-flow-step"><label>04</label><span>用户领奖</span><b>30 天内手动领取</b></div></div><p>单轮结算按 2 小时进行；每累计 12 轮，会汇总成一个 24 小时股王周期。</p></article>
          <article class="rules-visual-card"><strong>分奖结构、领取与 24 小时股王</strong><div class="rules-pie-legend"><span><em class="c1"></em>第1名 50%</span><span><em class="c2"></em>第2名 30%</span><span><em class="c3"></em>第3名 20%</span></div><div class="rules-allocation"><i class="c1" style="width:50%"></i><i class="c2" style="width:30%"></i><i class="c3" style="width:20%"></i></div><div class="rules-eq"><span>24 小时股王</span><b>按 12 轮累计结果产生</b></div><div class="rules-eq"><span>统计口径</span><b>看日周期积分、第一名次数与累计热度</b></div><p>若军团长在锁榜快照时满足资格，可额外拿到该军团奖励的 10%；所有奖励均需用户手动领取，过期回库。</p></article>
        </section>
        <section class="rules-leader-strip"><div><span>首次绑定</span><b>首次入团免费，任选 1-5 军团</b></div><div><span>换团路径</span><b>只能通过换仓并贡献完成</b></div><div><span>24 小时股王</span><b>每 12 轮汇总一次日周期结果</b></div></section>
      </div>
    `);
  ...
  function openJoinLegionModal() {
    const current = Number(state.userData?.currentLegion || 1);
    const legionNames = { 1: "AI军团", 2: "电车军团", 3: "比特军团", 4: "交易所军团", 5: "军工AI军团" };
    const options = [1, 2, 3, 4, 5]
      .map((id) => `<button type="button" class="chip legion-option ${id === current ? "active" : ""}" data-legion-option="${id}"><span class="legion-option-top"><span class="legion-option-id">0${id}</span><span class="legion-option-dot"></span></span><span class="legion-option-name">${legionNames[id]}</span><span class="legion-option-sub">选择军团 ${id}</span></button>`)
      .join("");
    openModal("加入军团", `<div class="legion-picker-shell"><div class="legion-picker-head"><span>选择你的军团</span><p>首次加入后将绑定当前军团，后续切换请使用换仓并贡献。</p></div><div class="legion-picker">${options}</div><input id="joinLegionInput" type="hidden" value="${current}" /></div><div class="inline-actions legion-picker-actions"><button id="joinLegionConfirmBtn" class="primary-btn">确认加入军团</button></div>`);
  ...
  async function openUpgradeRankModal() {
    ...
    const qualifySub = nextRank < 2 ? "升级后仍为基础热度阶段" : holdingOk ? "升级后满足持仓门槛" : "升级后仍需持仓 ≥ 5,000,000 主币才计入有效加权";
  ...
  function openFaqModal() {
    openModal("FAQ", `<div class="faq-modal"><div class="faq-intro"><strong>用户常见问题</strong><p>下面是最容易误解的规则与交互说明，口径均以前端当前接入合约为准。</p></div><div class="faq-list"><article class="faq-item"><h3>1. 为什么我贡献了 BNB，本季有效贡献还是 0？</h3><p>因为有效贡献要求同时满足两项：当前持仓至少 5,000,000 主币，且军衔达到分析师及以上；否则只会记入原始热度，不进入有效加权排名。</p></article><article class="faq-item"><h3>2. BNB 打榜的热度怎么计算？</h3><p>直接贡献按 BNB × 1,000,000 计算热度，例如 0.01 BNB 会增加 10,000 热度。</p></article><article class="faq-item"><h3>3. 为什么锁榜期贡献没有立刻计入当前轮？</h3><p>每轮最后 5 分钟进入锁榜期，锁榜期内的贡献会计入下一轮，防止最后时刻改写当前赛果。</p></article><article class="faq-item"><h3>4. 首次加入军团和后续换团有什么区别？</h3><p>首次加入免费，任选 1-5 军团；后续换团只能通过换仓并贡献完成，最低 0.005 BNB，税后 80% 计入新军团热度。</p></article><article class="faq-item"><h3>5. 升级军衔除了名字变化，还有什么作用？</h3><p>升级费用会转成热度，且分析师及以上才能进入有效加权；军衔越高，加权倍率越高，最高可到 200%。</p></article><article class="faq-item"><h3>6. 奖励为什么查询不到或显示 0？</h3><p>常见原因包括：赛季还没结算、你查错赛季或军团、你该赛季没有有效加权贡献、当前持仓不足，或奖励已过期/已领取。</p></article><article class="faq-item"><h3>7. 赛季奖励怎么分？</h3><p>每轮只释放当前奖池的 40%，再由前三军团按 50% / 30% / 20% 分配；如果只有 1 个或 2 个军团入榜，未分出的部分继续滚存。</p></article><article class="faq-item"><h3>8. 为什么还要手动领取奖励？</h3><p>合约采用手动领取模式，赛季结算后会开启 30 天领取窗口；过期未领的奖励会按规则回库。</p></article><article class="faq-item"><h3>9. 军团长怎么产生，怎么挑战？</h3><p>满足投资总监及以上且本军团历史贡献更高的用户，可以挑战军团长；成功后成为新军团长并获得 30 分钟保护期。</p></article><article class="faq-item"><h3>10. 为什么排行榜和奖励查询有时看起来不同步？</h3><p>排行榜是实时热度，奖励查询看的是已结算赛季结果；如果当前轮未结束或未结算，排行榜变化不会立刻体现在奖励结果里。</p></article></div></div>`);
    setNavActive("faq");
  }
